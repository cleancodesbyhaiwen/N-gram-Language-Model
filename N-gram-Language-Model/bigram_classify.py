...
print("Counting bigram frequencies of training set...")
gpt_bigram_freq = {}
for i in range(len(gpt_train)-1):
    if str(gpt_train[i] + ' ' + gpt_train[i+1]) in gpt_bigram_freq:
        gpt_bigram_freq[gpt_train[i] + ' ' + gpt_train[i+1]] += 1
    else:
        gpt_bigram_freq[gpt_train[i] + ' ' +  gpt_train[i+1]] = 1
gpt_words_freq = {}
...
for i in range(len(hum_train)-1):
    if str(hum_train[i] + ' ' + hum_train[i+1]) in hum_bigram_freq:
        hum_bigram_freq[hum_train[i] + ' ' +  hum_train[i+1]] += 1
    else:
        hum_bigram_freq[hum_train[i] + ' ' +  hum_train[i+1]] = 1
hum_words_freq = {}
...
print("Counting bigram frequencies of test set...")
gpt_bigram_freq_test = {}
for i in range(len(gpt_test)-1):
    if str(gpt_test[i] + ' ' + gpt_test[i+1]) in gpt_bigram_freq_test:
        gpt_bigram_freq_test[gpt_test[i] + ' ' + gpt_test[i+1]] += 1
    else:
        gpt_bigram_freq_test[gpt_test[i] + ' ' +  gpt_test[i+1]] = 1
gpt_words_freq_test = {}
for i in range(len(gpt_test)):
    if gpt_test[i] in gpt_words_freq_test:
        gpt_words_freq_test[gpt_test[i]] += 1
    else:
        gpt_words_freq_test[gpt_test[i]] = 1

hum_bigram_freq_test = {}
for i in range(len(hum_test)-1):
    if str(hum_test[i] + ' ' + hum_test[i+1]) in hum_bigram_freq_test:
        hum_bigram_freq_test[hum_test[i] + ' ' +  hum_test[i+1]] += 1
    else:
        hum_bigram_freq_test[hum_test[i] + ' ' +  hum_test[i+1]] = 1
hum_words_freq_test = {}
for i in range(len(hum_test)):
    if hum_test[i] in hum_words_freq_test:
        hum_words_freq_test[hum_test[i]] += 1
    else:
        hum_words_freq_test[hum_test[i]] = 1
...
oov_gpt = 0
oov_hum = 0
for key in gpt_bigram_freq_test:
    if key not in gpt_bigram_freq:
        oov_gpt += 1
for key in hum_bigram_freq_test:
    if key not in hum_bigram_freq:
        oov_hum += 1

oov_rate_gpt = oov_gpt / len(gpt_bigram_freq_test)
hum_rate_gpt = oov_hum / len(hum_bigram_freq_test)
...
gpt_test_chunks = gpt_test_chunks[:500]
hum_test_chunks = hum_test_chunks[:500]
...
for gpt_sentence in gpt_test_chunks:
    ...
    pr_from_gpt = pr_gpt * bigram_pr(gpt_sentence, gpt_bigram_freq, gpt_words_freq)
    pr_from_hum = pr_gpt * bigram_pr(gpt_sentence, hum_bigram_freq, hum_words_freq)
...
for hum_sentence in hum_test_chunks:
    print ("correct: " + str(correct) + " false: " + str(false), end="\r")
    pr_from_gpt = pr_gpt * bigram_pr(hum_sentence, gpt_bigram_freq, gpt_words_freq)
    pr_from_hum = pr_gpt * bigram_pr(hum_sentence, hum_bigram_freq, hum_words_freq)
    if pr_from_gpt <= pr_from_hum:
        correct += 1
    else:
        false += 1
